...
class UnderReplicatedCmd(KafkaCheckCmd):
    ...
    def build_subparser(self, subparsers):
        subparser = subparsers.add_parser(
            'under_replicated',
            description='Check under replicated partitions for all brokers in cluster.',
            help='This command will sum all under replicated partitions for each broker if any.'
                 'It will query jolokia port for receive this data.',
        )
        subparser.add_argument(
            '--first-broker-only',
            action='store_true',
            help='If this parameter is specified, it will do nothing and succeed on '
                 'not first brokers from Kafka cluster. Set --broker-id to -1 to read broker-id '
                 'from --data-path. Default: %(default)s',
        )
        subparser.add_argument(
            '--jolokia-port',
    ...
    def run_command(self):
        ...
        count = get_under_replicated(broker_list, self.args.jolokia_port)

        if count == 0:
            return status_code.OK, 'No under replicated partitions'
        else:
            msg = "{under_replicated} under replicated partitions".format(under_replicated=count)
...
def get_under_replicated_from_broker(host, port):
    ...
    :returns int: number of under replicated partitions for the broker
    """
    ...
    return sum([int(val['Value']) for val in under_replicated.values()])
...
def get_under_replicated(broker_list, port):
    ...
    :param int port: jolokia port for query under replicated partitions number
    :returns int: total number of under replicated partitions
    """
    return sum(
        [get_under_replicated_from_broker(broker_info['host'], port)
